/* =============================================
   CRS365 AI Tool Matcher - Application Logic
   GitHub Pages / Static Version
   ============================================= */

/*
  NOTE - Brevo Integration:
  Brevo API keys must never be exposed in browser-side code. This app sends
  lead submissions to a small serverless proxy, which then creates or updates
  the contact in Brevo. See workers/brevo-proxy.js for the Cloudflare Worker
  template and deployment notes.
*/

// Set this to your deployed Cloudflare Worker URL to enable Brevo sync.
// Leave empty string to disable (submissions still saved to localStorage).
const BREVO_PROXY_URL = 'https://crs365-tool-matcher-api.crshoop300.workers.dev';

// ============ TOOL DATABASE (embedded) ============
const TOOL_DATA_REVIEWED = 'July 8, 2026';
const TOOLS_DB = [
{"id":1,"name":"ChatGPT Business","category":"AI & Custom GPTs","description":"Shared business workspace for ChatGPT with advanced reasoning, file analysis, projects, custom GPTs, image generation, and team administration.","whyMatch":"A strong first AI layer for writing, research, analysis, documents, custom assistants, and repeatable knowledge work.","pricing":"Business plan; per-user monthly or annual pricing","priceValue":25,"hasFreeTier":false,"integrationEase":"Easy","bestFor":"Broad AI assistance across daily team work","pros":["General-purpose AI coverage","Projects, files, data analysis, and custom GPTs","Useful across sales, ops, marketing, and admin"],"cons":["Needs internal usage rules","Outputs still need review","Automation usually needs another workflow tool"],"integrations":["Google Workspace","Microsoft 365","Slack","Zapier","REST APIs"],"challenges":["Content creation at scale","Data analysis & reporting","Customer support efficiency","Team collaboration across tools","Manual data entry & repetitive tasks"],"companySizes":["Solo/Freelancer","2-10 employees","11-50","51-200","200+"],"industries":["All"]},
{"id":2,"name":"Claude Team","category":"AI & Custom GPTs","description":"Claude for teams with long-context reasoning, organization controls, Microsoft 365 connectors, enterprise search, and seat administration.","whyMatch":"Excellent when work depends on long documents, careful writing, research, and nuanced analysis.","pricing":"$20-$25/seat/mo Standard with 5-seat minimum; Premium $100-$125/seat/mo","priceValue":100,"hasFreeTier":false,"integrationEase":"Easy","bestFor":"Document-heavy teams that need careful reasoning and drafting","pros":["Strong long-document analysis","Good writing and reasoning quality","Team administration and connector controls"],"cons":["Minimum team size applies","Premium usage gets expensive","Not a full automation platform by itself"],"integrations":["Microsoft 365","Slack","Google Workspace","REST APIs"],"challenges":["Content creation at scale","Data analysis & reporting"],"companySizes":["2-10 employees","11-50","51-200"],"industries":["Professional Services","Finance","Healthcare","Education","Technology/SaaS","Marketing/Agency","Other"]},
{"id":3,"name":"Microsoft 365 Copilot Business","category":"AI & Custom GPTs","description":"Copilot inside Word, Excel, PowerPoint, Outlook, Teams, and work chat, with prebuilt agents and adoption analytics.","whyMatch":"Best when the business already runs on Microsoft 365 and wants AI embedded in the tools people use all day.","pricing":"$21/user/mo list; promo shown at $18/user/mo paid yearly","priceValue":21,"hasFreeTier":false,"integrationEase":"Easy","bestFor":"Microsoft 365 teams that want AI in Office, Outlook, Teams, and work data","pros":["Native Microsoft 365 experience","Works across Word, Excel, PowerPoint, Outlook, and Teams","Includes research, analyst, and facilitator agents"],"cons":["Requires eligible Microsoft 365 Business plan","Less useful for Google-first companies","Rollout needs permissions and governance planning"],"integrations":["Microsoft 365"],"challenges":["Content creation at scale","Data analysis & reporting","Team collaboration across tools","Manual data entry & repetitive tasks"],"companySizes":["2-10 employees","11-50","51-200","200+"],"industries":["All"]},
{"id":4,"name":"Google Workspace with Gemini","category":"AI & Custom GPTs","description":"Gemini AI in Gmail, Docs, Meet, Sheets, Slides, Drive, and the Gemini app inside Google Workspace plans.","whyMatch":"Best for Google-first teams that want AI in email, docs, meetings, spreadsheets, and everyday collaboration.","pricing":"Business Plus listed at $22/user/mo; Gemini included across Workspace tiers","priceValue":22,"hasFreeTier":false,"integrationEase":"Easy","bestFor":"Google Workspace companies that want native AI assistance","pros":["Native Gmail, Docs, Meet, Drive, and Sheets experience","Simple for Google-first teams","NotebookLM and Gemini app access on business plans"],"cons":["Less useful for Microsoft-heavy companies","Admin/security depends on Workspace tier","May still need automation tools for cross-app workflows"],"integrations":["Google Workspace"],"challenges":["Content creation at scale","Data analysis & reporting","Team collaboration across tools","Customer support efficiency"],"companySizes":["Solo/Freelancer","2-10 employees","11-50","51-200","200+"],"industries":["All"]},
{"id":5,"name":"Zapier Automation Platform","category":"Workflow Automation","description":"No-code automation across thousands of apps, plus Zapier Agents, Chatbots, Canvas, MCP, AI by Zapier, and AI-assisted workflow building.","whyMatch":"A practical first automation layer when tools are disconnected and people are still copying data by hand.","pricing":"Free tier plus paid plans by task volume","priceValue":20,"hasFreeTier":true,"integrationEase":"Easy","bestFor":"Quick no-code automations between business apps","pros":["Large app ecosystem","Beginner-friendly builder","AI agents, chatbots, and AI tools built in"],"cons":["Task volume can raise cost","Complex branching needs careful design","Important automations need monitoring"],"integrations":["Google Workspace","Microsoft 365","Slack","Salesforce","HubSpot","REST APIs"],"challenges":["Manual data entry & repetitive tasks","Disconnected tools/systems","Lead management & follow-up","Customer support efficiency","Team collaboration across tools"],"companySizes":["Solo/Freelancer","2-10 employees","11-50","51-200"],"industries":["All"]},
{"id":6,"name":"Make","category":"Workflow Automation","description":"Visual workflow automation with routers, filters, API access, 3,000+ apps, and credit-based pricing that scales with usage.","whyMatch":"Strong when workflows need branching logic, visual debugging, and more control than simple two-step automations.","pricing":"Free up to 1,000 credits/mo; Core from $12/mo for 10k credits","priceValue":12,"hasFreeTier":true,"integrationEase":"Moderate","bestFor":"Visual multi-step automations with conditional logic","pros":["Visual scenario builder","Routers and filters for advanced logic","Low entry cost"],"cons":["More setup learning than Zapier","Credit usage requires monitoring","Can become complex without documentation"],"integrations":["Google Workspace","Microsoft 365","Slack","Airtable","REST APIs"],"challenges":["Manual data entry & repetitive tasks","Disconnected tools/systems","Data analysis & reporting","Customer support efficiency"],"companySizes":["Solo/Freelancer","2-10 employees","11-50","51-200"],"industries":["All"]},
{"id":7,"name":"n8n Cloud","category":"Workflow Automation","description":"Technical workflow automation with unlimited steps, AI workflow builder credits, cloud hosting, and a self-hosting path for more control.","whyMatch":"Best for technical operators who want powerful workflows, custom API calls, and fewer limits on workflow complexity.","pricing":"Starter 20 EUR/mo annually; Pro 50 EUR/mo annually","priceValue":23,"hasFreeTier":false,"integrationEase":"Advanced","bestFor":"Technical teams building flexible automation and API workflows","pros":["Unlimited workflow steps","Custom code and API flexibility","Self-hosting option"],"cons":["Not ideal for non-technical teams","Requires workflow design discipline","Cloud pricing is based on executions"],"integrations":["Google Workspace","Microsoft 365","Slack","REST APIs"],"challenges":["Manual data entry & repetitive tasks","Disconnected tools/systems","Data analysis & reporting"],"companySizes":["2-10 employees","11-50","51-200"],"industries":["Technology/SaaS","Professional Services","Finance","Marketing/Agency","Manufacturing","Other"]},
{"id":8,"name":"Microsoft Power Automate","category":"Workflow Automation","description":"Low-code automation for cloud flows, attended desktop RPA, process mining, custom connectors, and Microsoft-first workflows.","whyMatch":"Natural fit when the business uses Microsoft 365 and wants automations inside Office, Teams, SharePoint, and desktop processes.","pricing":"Premium $15/user/mo paid yearly; process bots from $150/bot/mo","priceValue":15,"hasFreeTier":true,"integrationEase":"Moderate","bestFor":"Microsoft-heavy organizations that need workflow automation and RPA","pros":["Deep Microsoft 365 integration","Cloud flows and desktop RPA","Strong governance options"],"cons":["Licensing can be confusing","Best value inside Microsoft ecosystems","Some flows need admin support"],"integrations":["Microsoft 365","Slack","REST APIs"],"challenges":["Manual data entry & repetitive tasks","Disconnected tools/systems","Team collaboration across tools"],"companySizes":["2-10 employees","11-50","51-200","200+"],"industries":["All"]},
{"id":9,"name":"Airtable AI & Automations","category":"Workflow Automation","description":"Flexible app-building platform combining databases, interfaces, automations, AI app building, and AI agents.","whyMatch":"Great when the business lives in spreadsheets and needs a more structured operational database without heavy custom software.","pricing":"Free plan; Team $20/user/mo annually; Business $45/user/mo annually","priceValue":20,"hasFreeTier":true,"integrationEase":"Easy","bestFor":"Operational databases, lightweight apps, forms, and workflow hubs","pros":["Spreadsheet-like interface","Good internal app and form builder","Automations and AI in one platform"],"cons":["Needs clean data design to scale","Business plan gets expensive by seat","Complex processes may need external automation"],"integrations":["Google Workspace","Slack","Zapier","REST APIs"],"challenges":["Manual data entry & repetitive tasks","Disconnected tools/systems","Team collaboration across tools","Data analysis & reporting"],"companySizes":["Solo/Freelancer","2-10 employees","11-50","51-200"],"industries":["Marketing/Agency","Real Estate","Professional Services","Education","Manufacturing","Technology/SaaS","Other"]},
{"id":10,"name":"Notion Business","category":"AI & Custom GPTs","description":"Collaborative workspace with docs, databases, wikis, Notion Agent, AI meeting notes, enterprise search, and app connections.","whyMatch":"Strong when team knowledge, SOPs, notes, and lightweight project tracking are scattered across too many places.","pricing":"Plus $10/member/mo; Business $20/member/mo","priceValue":20,"hasFreeTier":true,"integrationEase":"Easy","bestFor":"Knowledge management, team docs, SOPs, projects, and AI-assisted workspaces","pros":["Simple docs and database workspace","Notion Agent and AI meeting notes on Business","Good for SOPs and internal knowledge"],"cons":["Can get messy without architecture","Not a full CRM or automation suite","Heavy databases can become hard to govern"],"integrations":["Google Workspace","Slack","GitHub","Zapier"],"challenges":["Team collaboration across tools","Content creation at scale"],"companySizes":["Solo/Freelancer","2-10 employees","11-50","51-200"],"industries":["All"]},
{"id":11,"name":"HubSpot CRM + Breeze","category":"CRM & Sales","description":"HubSpot customer platform with Smart CRM, sales tools, marketing/service hubs, Breeze AI agents, and credit-based AI features.","whyMatch":"A strong growth stack when leads, follow-up, sales visibility, and customer support all need to live in one place.","pricing":"Free CRM; Sales Hub Starter from $20/mo/seat monthly with annual discounts","priceValue":20,"hasFreeTier":true,"integrationEase":"Easy","bestFor":"Businesses that need CRM, sales follow-up, and service workflows","pros":["Free CRM entry point","Sales, marketing, and service in one platform","Breeze agents for support, prospecting, and data work"],"cons":["Professional tiers get expensive","Needs careful setup to avoid clutter","AI usage may depend on credits"],"integrations":["Google Workspace","Microsoft 365","Slack","Zapier","Salesforce"],"challenges":["Lead management & follow-up","Sales pipeline visibility","Customer support efficiency","Disconnected tools/systems"],"companySizes":["Solo/Freelancer","2-10 employees","11-50","51-200"],"industries":["Technology/SaaS","Professional Services","Marketing/Agency","Real Estate","Education","E-commerce/Retail","Other"]},
{"id":12,"name":"Pipedrive CRM","category":"CRM & Sales","description":"Sales-focused CRM with AI-assisted reporting, pipeline management, automations, lead routing, email tools, and deal visibility.","whyMatch":"Best for teams that mainly need a clear sales pipeline and disciplined follow-up rather than a full marketing platform.","pricing":"Lite $14/seat/mo annually; Growth $39; Premium $59","priceValue":14,"hasFreeTier":false,"integrationEase":"Easy","bestFor":"Sales teams that want simple pipeline management and follow-up automation","pros":["Clear pipeline UI","AI-powered report creation on entry plan","500+ integrations"],"cons":["Less all-in-one than HubSpot","LeadBooster and add-ons cost extra","Advanced automation starts on higher tiers"],"integrations":["Google Workspace","Microsoft 365","Slack","Zapier"],"challenges":["Lead management & follow-up","Sales pipeline visibility"],"companySizes":["Solo/Freelancer","2-10 employees","11-50","51-200"],"industries":["Professional Services","Real Estate","Technology/SaaS","Marketing/Agency","Manufacturing","Other"]},
{"id":13,"name":"Salesforce Sales Cloud","category":"CRM & Sales","description":"Enterprise CRM with Sales Cloud, built-in AI options, Agentforce, pipeline management, forecasting, and broad ecosystem depth.","whyMatch":"Better for larger teams that need enterprise CRM controls, customization, forecasting, and multi-team sales operations.","pricing":"Starter Suite $25/user/mo; Pro Suite $100; Enterprise $175","priceValue":25,"hasFreeTier":true,"integrationEase":"Advanced","bestFor":"Scaling sales organizations with complex CRM and governance needs","pros":["Deep CRM ecosystem","Enterprise customization and controls","Agentforce and AI add-ons available"],"cons":["Implementation can be heavy","Expensive at higher tiers","Usually needs admin expertise"],"integrations":["Google Workspace","Microsoft 365","Slack","Zapier","REST APIs"],"challenges":["Lead management & follow-up","Sales pipeline visibility","Customer support efficiency","Data analysis & reporting"],"companySizes":["11-50","51-200","200+"],"industries":["Technology/SaaS","Finance","Healthcare","Manufacturing","Professional Services","Other"]},
{"id":14,"name":"Intercom + Fin AI Agent","category":"CRM & Sales","description":"AI-first customer service platform with teammate seats, messaging channels, Fin AI Agent, workflows, and outcome-based AI pricing.","whyMatch":"Strong when support volume is growing and AI should answer common questions before a human gets involved.","pricing":"Seat pricing plus Fin at $0.99 per resolved outcome","priceValue":39,"hasFreeTier":false,"integrationEase":"Moderate","bestFor":"Customer support teams that need AI deflection and live support workflows","pros":["Fin AI Agent included with Intercom plans","Outcome-based AI pricing","Strong support inbox and messaging workflows"],"cons":["Usage can scale cost quickly","Best for support-heavy businesses","Requires knowledge base and workflow setup"],"integrations":["Google Workspace","Slack","Zapier","Salesforce","REST APIs"],"challenges":["Customer support efficiency","Lead management & follow-up"],"companySizes":["2-10 employees","11-50","51-200","200+"],"industries":["Technology/SaaS","E-commerce/Retail","Education","Professional Services","Other"]},
{"id":15,"name":"Jasper","category":"AI & Custom GPTs","description":"AI marketing platform with brand voice, knowledge assets, marketing workflow agents, canvas-based creation, and custom AI agents on Business.","whyMatch":"Best when content volume and brand consistency matter more than general-purpose AI experimentation.","pricing":"Pro $69/seat/mo monthly or $59/seat/mo yearly; Business custom","priceValue":59,"hasFreeTier":false,"integrationEase":"Easy","bestFor":"Marketing teams that need on-brand content workflows at scale","pros":["Purpose-built for marketing teams","Brand voices and knowledge assets","Business plan supports complex marketing agents"],"cons":["Not ideal for non-marketing workflows","Higher cost than general AI assistants","Still requires editorial review"],"integrations":["Google Workspace","Zapier","REST APIs"],"challenges":["Content creation at scale"],"companySizes":["2-10 employees","11-50","51-200","200+"],"industries":["Marketing/Agency","E-commerce/Retail","Technology/SaaS","Professional Services","Other"]},
{"id":16,"name":"monday work management","category":"Workflow Automation","description":"Work management platform with boards, automations, dashboards, AI credits, AI agents, notetaker, Sidekick assistant, and AI columns.","whyMatch":"Strong when collaboration, handoffs, project status, and team accountability are the core operational pain points.","pricing":"Basic from $9/seat/mo annually; AI credits included by plan","priceValue":9,"hasFreeTier":true,"integrationEase":"Easy","bestFor":"Visual project, operations, and workflow management","pros":["Visual boards and dashboards","AI agents, notetaker, Sidekick, and AI columns","Good for cross-functional teams"],"cons":["Can sprawl without workspace rules","Some AI capacity is credit-based","Advanced views and automation require higher plans"],"integrations":["Google Workspace","Microsoft 365","Slack","Zapier"],"challenges":["Team collaboration across tools","Disconnected tools/systems","Manual data entry & repetitive tasks","Data analysis & reporting"],"companySizes":["2-10 employees","11-50","51-200","200+"],"industries":["Marketing/Agency","Manufacturing","Professional Services","Education","Real Estate","Other"]},
{"id":17,"name":"Power BI","category":"Analytics & BI","description":"Microsoft business intelligence with free account, Pro sharing, Premium per user, Fabric integration, advanced AI, and governed reporting.","whyMatch":"Best for Microsoft-heavy teams that need dashboards, reporting, and shared business intelligence.","pricing":"Free account; Pro $14/user/mo paid yearly; Premium Per User $24/user/mo","priceValue":14,"hasFreeTier":true,"integrationEase":"Moderate","bestFor":"Microsoft data reporting, dashboards, and executive visibility","pros":["Strong Microsoft and Fabric integration","Affordable Pro license","Good for recurring reports and dashboards"],"cons":["Data modeling takes skill","Sharing usually requires paid licenses","Best results need clean source data"],"integrations":["Microsoft 365","REST APIs"],"challenges":["Data analysis & reporting","Sales pipeline visibility"],"companySizes":["2-10 employees","11-50","51-200","200+"],"industries":["All"]},
{"id":18,"name":"Looker Studio","category":"Analytics & BI","description":"Google reporting and dashboard tool for creating shareable reports from Google and connected data sources.","whyMatch":"Best free dashboard starting point for Google-first teams that need basic reporting without a BI budget.","pricing":"Free","priceValue":0,"hasFreeTier":true,"integrationEase":"Easy","bestFor":"Google-first reporting dashboards and lightweight business visibility","pros":["Free to start","Easy Google Analytics, Sheets, and Ads reporting","Shareable dashboards"],"cons":["Less powerful than paid BI platforms","Connector quality varies","Can slow down with large data sets"],"integrations":["Google Workspace","REST APIs"],"challenges":["Data analysis & reporting"],"companySizes":["Solo/Freelancer","2-10 employees","11-50"],"industries":["Marketing/Agency","E-commerce/Retail","Professional Services","Real Estate","Other"]},
{"id":19,"name":"Tableau","category":"Analytics & BI","description":"AI-powered analytics platform with Tableau Cloud, Desktop/Prep, Tableau Pulse, Tableau Agent, and enterprise analytics options.","whyMatch":"Useful when the company needs polished, scalable BI and has the data maturity to support it.","pricing":"Tableau Standard from $15/user/mo billed annually; Enterprise from $35","priceValue":15,"hasFreeTier":false,"integrationEase":"Advanced","bestFor":"Governed analytics and strong visualization","pros":["Strong dashboards and visual exploration","Tableau Pulse and agentic analytics options","Good enterprise deployment flexibility"],"cons":["Needs data expertise","Annual contract model","Can be overkill for very small teams"],"integrations":["Google Workspace","Microsoft 365","Salesforce","REST APIs"],"challenges":["Data analysis & reporting","Sales pipeline visibility"],"companySizes":["11-50","51-200","200+"],"industries":["Finance","Healthcare","Manufacturing","Technology/SaaS","Education","Other"]},
{"id":20,"name":"Fireflies.ai","category":"AI & Custom GPTs","description":"AI meeting assistant for transcription, summaries, video recording, action items, voice agents, analytics, and integrations across Zoom, Google Meet, and Teams.","whyMatch":"A practical AI win when meetings create lost details, missed follow-ups, and poor handoff visibility.","pricing":"Free; Pro $10/seat/mo annually; Business $19/seat/mo annually","priceValue":10,"hasFreeTier":true,"integrationEase":"Easy","bestFor":"Meeting notes, follow-ups, conversation intelligence, and team memory","pros":["Free forever option","Unlimited transcription and summaries on paid tiers","Works with Zoom, Google Meet, and Microsoft Teams"],"cons":["Meeting-focused rather than full automation","Requires consent and meeting policy clarity","Storage and AI credits vary by plan"],"integrations":["Google Workspace","Microsoft 365","Slack","REST APIs"],"challenges":["Team collaboration across tools","Lead management & follow-up","Customer support efficiency"],"companySizes":["Solo/Freelancer","2-10 employees","11-50","51-200"],"industries":["Professional Services","Technology/SaaS","Real Estate","Education","Healthcare","Marketing/Agency","Other"]},
{"id":21,"name":"Relevance AI","category":"AI & Custom GPTs","description":"Enterprise AI workforce platform with unlimited agents, tools, users, workforces, 2,000+ integrations, custom actions, evaluations, and governance.","whyMatch":"A candidate for larger teams that want agentic workflows tied to sales, marketing, operations, research, or support.","pricing":"Enterprise/custom pricing","priceValue":500,"hasFreeTier":false,"integrationEase":"Advanced","bestFor":"AI agent workforces for larger teams with defined processes","pros":["Unlimited agents and workforces on enterprise plan","2,000+ integrations","SSO, RBAC, audit logs, and agent evaluations"],"cons":["Enterprise sales motion","Needs clear process design before purchase","Overkill for small teams testing first automations"],"integrations":["Google Workspace","Microsoft 365","Slack","REST APIs"],"challenges":["Manual data entry & repetitive tasks","Lead management & follow-up","Customer support efficiency","Data analysis & reporting"],"companySizes":["51-200","200+"],"industries":["Technology/SaaS","Finance","Marketing/Agency","Professional Services","Other"]}
];

// ============ STATE ============
let currentStep = 1;
const totalSteps = 6;
let formData = {
    companySize: '',
    industry: '',
    challenges: [],
    budget: '',
    currentTools: [],
    techLevel: ''
};
let scoredTools = [];
let selectedForCompare = [];
let activeFilter = 'All';
let activeSort = 'score';

// ============ DOM REFS ============
const sections = {
    hero: document.getElementById('hero-section'),
    assessment: document.getElementById('assessment-section'),
    email: document.getElementById('email-section'),
    results: document.getElementById('results-section')
};

// ============ NAVIGATION ============
function showSection(name) {
    Object.values(sections).forEach(s => s.classList.remove('active'));
    sections[name].classList.add('active');
    window.scrollTo({ top: 0, behavior: 'smooth' });
}

// Start button
document.getElementById('start-btn').addEventListener('click', () => {
    showSection('assessment');
    showStep(1);
});

// ============ FORM STEPS ============
function showStep(step) {
    currentStep = step;
    document.querySelectorAll('.form-step').forEach(s => {
        s.classList.remove('active');
        s.style.display = 'none';
    });
    const activeStep = document.querySelector(`.form-step[data-step="${step}"]`);
    if (activeStep) {
        activeStep.style.display = 'block';
        // Trigger animation
        activeStep.offsetHeight; // force reflow
        activeStep.classList.add('active');
    }

    // Update progress
    const fill = document.getElementById('progress-fill');
    fill.style.width = `${(step / totalSteps) * 100}%`;
    document.getElementById('progress-text').textContent = `Question ${step} of ${totalSteps}`;

    // Back button visibility
    const prevBtn = document.getElementById('prev-btn');
    prevBtn.style.visibility = step === 1 ? 'hidden' : 'visible';

    // Next button text
    const nextBtn = document.getElementById('next-btn');
    nextBtn.innerHTML = step === totalSteps 
        ? 'Finish <svg width="16" height="16" viewBox="0 0 20 20" fill="none"><path d="M4 10h12m0 0l-4-4m4 4l-4 4" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>'
        : 'Continue <svg width="16" height="16" viewBox="0 0 20 20" fill="none"><path d="M4 10h12m0 0l-4-4m4 4l-4 4" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>';

    validateStep();
}

// Next / Prev
document.getElementById('next-btn').addEventListener('click', () => {
    saveStepData();
    if (currentStep < totalSteps) {
        showStep(currentStep + 1);
    } else {
        // Go to email gate
        showSection('email');
    }
});

document.getElementById('prev-btn').addEventListener('click', () => {
    if (currentStep > 1) showStep(currentStep - 1);
});

// Save form step data
function saveStepData() {
    switch(currentStep) {
        case 1:
            const sizeRadio = document.querySelector('input[name="companySize"]:checked');
            if (sizeRadio) formData.companySize = sizeRadio.value;
            break;
        case 2:
            const sel = document.getElementById('industry-select');
            if (sel.value === 'Other') {
                formData.industry = document.getElementById('other-industry').value || 'Other';
            } else {
                formData.industry = sel.value;
            }
            break;
        case 3:
            formData.challenges = Array.from(document.querySelectorAll('input[name="challenges"]:checked')).map(c => c.value);
            break;
        case 4:
            const budgetRadio = document.querySelector('input[name="budget"]:checked');
            if (budgetRadio) formData.budget = budgetRadio.value;
            break;
        case 5:
            formData.currentTools = Array.from(document.querySelectorAll('input[name="currentTools"]:checked')).map(c => c.value);
            break;
        case 6:
            const techRadio = document.querySelector('input[name="techLevel"]:checked');
            if (techRadio) formData.techLevel = techRadio.value;
            break;
    }
}

// Validate step
function validateStep() {
    const nextBtn = document.getElementById('next-btn');
    let valid = false;
    switch(currentStep) {
        case 1:
            valid = !!document.querySelector('input[name="companySize"]:checked');
            break;
        case 2:
            const sel = document.getElementById('industry-select');
            valid = sel.value !== '';
            if (sel.value === 'Other') {
                valid = document.getElementById('other-industry').value.trim().length > 0;
            }
            break;
        case 3:
            valid = document.querySelectorAll('input[name="challenges"]:checked').length > 0;
            break;
        case 4:
            valid = !!document.querySelector('input[name="budget"]:checked');
            break;
        case 5:
            valid = document.querySelectorAll('input[name="currentTools"]:checked').length > 0;
            break;
        case 6:
            valid = !!document.querySelector('input[name="techLevel"]:checked');
            break;
    }
    nextBtn.disabled = !valid;
}

// Listen for input changes to validate
document.querySelectorAll('input[type="radio"], input[type="checkbox"]').forEach(input => {
    input.addEventListener('change', validateStep);
});

document.getElementById('industry-select').addEventListener('change', function() {
    const wrapper = document.getElementById('other-industry-wrapper');
    if (this.value === 'Other') {
        wrapper.classList.remove('hidden');
    } else {
        wrapper.classList.add('hidden');
    }
    validateStep();
});

document.getElementById('other-industry').addEventListener('input', validateStep);

// Other tools toggle
document.querySelectorAll('input[name="currentTools"]').forEach(cb => {
    cb.addEventListener('change', () => {
        const otherCb = document.querySelector('input[name="currentTools"][value="Other"]');
        const wrapper = document.getElementById('other-tools-wrapper');
        if (otherCb && otherCb.checked) {
            wrapper.classList.remove('hidden');
        } else {
            wrapper.classList.add('hidden');
        }
        validateStep();
    });
});

// ============ TRACKING HELPERS ============
function getTrackingParams() {
    const params = new URLSearchParams(window.location.search);
    return {
        utm_source: params.get('utm_source') || '',
        utm_medium: params.get('utm_medium') || '',
        utm_campaign: params.get('utm_campaign') || '',
        utm_content: params.get('utm_content') || '',
        utm_term: params.get('utm_term') || '',
        referrer: document.referrer || ''
    };
}

// ============ EMAIL FORM SUBMISSION ============
document.getElementById('email-form').addEventListener('submit', async function(e) {
    e.preventDefault();

    const name = document.getElementById('user-name').value.trim();
    const email = document.getElementById('user-email').value.trim();
    const company = document.getElementById('user-company').value.trim();
    const emailConsent = document.getElementById('newsletter-opt').checked;

    if (!name || !email || !emailConsent) return;

    // Show loading
    const submitBtn = this.querySelector('button[type="submit"]');
    const origHTML = submitBtn.innerHTML;
    submitBtn.innerHTML = '<div class="loading-spinner" style="width:20px;height:20px;border-width:2px;margin:0 auto;"></div>';
    submitBtn.disabled = true;

    // Save to formData
    formData.name = name;
    formData.email = email;
    formData.company = company;
    formData.emailConsent = emailConsent;

    // Calculate scores before syncing so Brevo receives the top recommendation.
    scoredTools = calculateScores();
    const topMatch = scoredTools[0] || {};
    const tracking = getTrackingParams();

    const submission = {
        id: Date.now(),
        name: formData.name,
        email: formData.email,
        company: formData.company,
        company_size: formData.companySize,
        industry: formData.industry,
        challenges: formData.challenges,
        budget: formData.budget,
        current_tools: formData.currentTools,
        tech_level: formData.techLevel,
        email_consent: formData.emailConsent,
        newsletter_opt_in: formData.emailConsent,
        top_match: topMatch.name || '',
        top_score: topMatch.score || '',
        submitted_at: new Date().toISOString(),
        ...tracking
    };

    // Save submission to localStorage as a browser-only backup.
    // NOTE: localStorage is per-browser/device. Use Brevo as the canonical list.
    try {
        const submissions = JSON.parse(localStorage.getItem('crs_submissions') || '[]');
        submissions.push(submission);
        localStorage.setItem('crs_submissions', JSON.stringify(submissions));
    } catch (err) {
        console.log('localStorage save failed (non-blocking):', err);
    }

    // Sync to Brevo via proxy (if configured)
    if (BREVO_PROXY_URL) {
        try {
            await fetch(BREVO_PROXY_URL, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(submission)
            });
        } catch (err) {
            console.log('Brevo proxy call failed (non-blocking):', err);
        }
    }

    // Simulate brief "analyzing" delay for UX
    await new Promise(r => setTimeout(r, 1200));

    submitBtn.innerHTML = origHTML;
    submitBtn.disabled = false;

    showSection('results');
    renderResults();
});
// ============ CRS FIT SCORE ALGORITHM ============
function calculateScores() {
    const results = TOOLS_DB.map(tool => {
        const challengeScore = getChallengeScore(tool, formData.challenges);
        const budgetScore = getBudgetScore(tool, formData.budget);
        const techScore = getTechScore(tool.integrationEase, formData.techLevel);
        const ecosystemScore = getEcosystemScore(tool, formData.currentTools);
        const sizeScore = getCompanySizeScore(tool, formData.companySize);
        const industryScore = getIndustryScore(tool, formData.industry);

        const score = Math.max(0, Math.min(100, Math.round(
            challengeScore + budgetScore + techScore + ecosystemScore + sizeScore + industryScore
        )));

        return {
            ...tool,
            score,
            scoreBreakdown: { challengeScore, budgetScore, techScore, ecosystemScore, sizeScore, industryScore }
        };
    });

    results.sort((a, b) => {
        if (b.score !== a.score) return b.score - a.score;
        if (a.priceValue !== b.priceValue) return a.priceValue - b.priceValue;
        return a.name.localeCompare(b.name);
    });
    return results;
}

function getChallengeScore(tool, challenges) {
    if (!challenges || challenges.length === 0) return 15;

    const matchedChallenges = challenges.filter(c => tool.challenges.includes(c));
    const challengeRatio = matchedChallenges.length / challenges.length;
    let score = challengeRatio * 30;

    if (matchedChallenges.length > 0 && tool.challenges.length <= 2) {
        score += 2; // Specialist bonus when the selected pain maps tightly to the tool.
    }

    return Math.min(30, Math.round(score));
}

function getBudgetScore(tool, budget) {
    const price = tool.priceValue;
    const hasFree = tool.hasFreeTier;

    switch(budget) {
        case 'Under $50/mo':
            if (hasFree || price === 0) return 20;
            if (price <= 20) return 18;
            if (price <= 50) return 14;
            if (price <= 100) return 7;
            return 2;
        case '$50-200/mo':
            if (price <= 0) return 17;
            if (price <= 50) return 20;
            if (price <= 100) return 17;
            if (price <= 200) return 14;
            return 6;
        case '$200-500/mo':
            if (price <= 50) return 17;
            if (price <= 200) return 20;
            if (price <= 500) return 17;
            return 10;
        case '$500-1000/mo':
            if (price <= 200) return 16;
            if (price <= 1000) return 20;
            return 14;
        case '$1000+/mo':
            if (price >= 200) return 20;
            return 17;
        default:
            return 10;
    }
}

function getTechScore(ease, techLevel) {
    switch(techLevel) {
        case 'Non-technical':
            if (ease === 'Easy') return 15;
            if (ease === 'Moderate') return 9;
            if (ease === 'Advanced') return 4;
            break;
        case 'Somewhat technical':
            if (ease === 'Easy') return 13;
            if (ease === 'Moderate') return 15;
            if (ease === 'Advanced') return 10;
            break;
        case 'Very technical':
            if (ease === 'Easy') return 10;
            if (ease === 'Moderate') return 13;
            if (ease === 'Advanced') return 15;
            break;
    }
    return 8;
}

function getEcosystemScore(tool, userTools) {
    if (!userTools || userTools.length === 0 || userTools.includes('None/Just getting started')) {
        return 8;
    }

    const relevantUserTools = userTools.filter(t => t !== 'Other' && t !== 'None/Just getting started');
    if (relevantUserTools.length === 0) return 8;

    const matchCount = relevantUserTools.filter(ut => tool.integrations.includes(ut)).length;
    if (matchCount === relevantUserTools.length) return 15;
    if (matchCount > 0) return 12;
    return 4;
}

function getCompanySizeScore(tool, companySize) {
    if (!companySize || !tool.companySizes || tool.companySizes.length === 0) return 5;
    if (tool.companySizes.includes(companySize)) return 10;

    const sizeOrder = ['Solo/Freelancer', '2-10 employees', '11-50', '51-200', '200+'];
    const userIndex = sizeOrder.indexOf(companySize);
    const closestToolIndex = tool.companySizes
        .map(size => sizeOrder.indexOf(size))
        .filter(index => index >= 0)
        .sort((a, b) => Math.abs(a - userIndex) - Math.abs(b - userIndex))[0];

    if (userIndex < 0 || closestToolIndex === undefined) return 5;

    const distance = Math.abs(userIndex - closestToolIndex);
    if (distance === 1) return 7;
    if (distance === 2) return 4;
    return 2;
}

function getIndustryScore(tool, industry) {
    if (!industry || !tool.industries || tool.industries.length === 0) return 5;
    if (tool.industries.includes('All')) return 8;
    if (tool.industries.includes(industry)) return 10;
    if (industry === 'Other') return 6;
    return 4;
}

// ============ RENDER RESULTS ============
function renderResults() {
    renderTopMatches();
    renderResultsGrid();
    renderSpotlight();
    renderCTA();
    
    // Animate scores after a brief delay
    setTimeout(animateScores, 300);
}

function getScoreColor(score) {
    if (score >= 80) return 'var(--green)';
    if (score >= 60) return 'var(--yellow)';
    return 'var(--orange)';
}

function getCategoryBadgeClass(category) {
    switch(category) {
        case 'Workflow Automation': return 'badge-automation';
        case 'AI & Custom GPTs': return 'badge-ai';
        case 'CRM & Sales': return 'badge-crm';
        case 'Analytics & BI': return 'badge-analytics';
        default: return 'badge-automation';
    }
}

function getEaseClass(ease) {
    switch(ease) {
        case 'Easy': return 'ease-easy';
        case 'Moderate': return 'ease-moderate';
        case 'Advanced': return 'ease-advanced';
        default: return '';
    }
}

// Top Matches Cards
function renderTopMatches() {
    const container = document.getElementById('top-matches');
    const top = scoredTools.slice(0, 4);
    const rankLabels = ['#1 Pick', '#2 Pick', '#3 Pick', '#4 Pick'];
    
    container.innerHTML = top.map((tool, i) => {
        const circumference = 2 * Math.PI * 33;
        const offset = circumference - (tool.score / 100) * circumference;
        const color = getScoreColor(tool.score);
        
        return `
        <div class="top-match-card rank-${i+1}" style="animation: fadeInUp ${0.4 + i*0.15}s ease-out both;">
            <span class="top-match-rank">${rankLabels[i]}</span>
            <h3 class="top-match-name">${tool.name}</h3>
            <span class="category-badge ${getCategoryBadgeClass(tool.category)}">${tool.category}</span>
            <div class="score-gauge">
                <svg width="80" height="80" viewBox="0 0 80 80">
                    <circle class="score-gauge-bg" cx="40" cy="40" r="33"/>
                    <circle class="score-gauge-fill" cx="40" cy="40" r="33"
                        stroke="${color}"
                        stroke-dasharray="${circumference}"
                        stroke-dashoffset="${circumference}"
                        data-target-offset="${offset}"/>
                </svg>
                <span class="score-gauge-text" style="color:${color}" data-target="${tool.score}">0</span>
            </div>
            <p class="top-match-why">${tool.whyMatch}</p>
            <span class="top-match-price">Starting at ${tool.pricing}</span>
        </div>`;
    }).join('');
}

// Full Results Grid
function renderResultsGrid() {
    const container = document.getElementById('results-grid');
    let filtered = scoredTools;
    
    if (activeFilter !== 'All') {
        filtered = scoredTools.filter(t => t.category === activeFilter);
    }

    // Sort
    switch(activeSort) {
        case 'score':
            filtered.sort((a, b) => b.score - a.score);
            break;
        case 'price-asc':
            filtered.sort((a, b) => a.priceValue - b.priceValue);
            break;
        case 'ease':
            const easeOrder = { 'Easy': 0, 'Moderate': 1, 'Advanced': 2 };
            filtered.sort((a, b) => easeOrder[a.integrationEase] - easeOrder[b.integrationEase]);
            break;
    }

    container.innerHTML = filtered.map((tool, i) => {
        const color = getScoreColor(tool.score);
        const isSelected = selectedForCompare.includes(tool.id);
        
        return `
        <div class="result-row" data-tool-id="${tool.id}" style="animation: fadeInUp ${0.1 + i*0.05}s ease-out both;">
            <div class="result-row-main" onclick="toggleExpand(this.parentElement)">
                <input type="checkbox" class="result-checkbox" 
                    ${isSelected ? 'checked' : ''} 
                    onclick="event.stopPropagation(); toggleCompare(${tool.id}, this.checked)">
                <div class="result-name-col">
                    <span class="result-name">${tool.name}</span>
                    <span class="result-category">${tool.category}</span>
                </div>
                <div class="result-score-col">
                    <div class="score-bar-container">
                        <div class="score-bar-fill" style="width: 0%; background: ${color};" data-target-width="${tool.score}%"></div>
                    </div>
                    <span class="score-bar-value" style="color: ${color};">${tool.score}</span>
                </div>
                <span class="result-pricing">${tool.pricing}</span>
                <span class="result-ease ${getEaseClass(tool.integrationEase)}">${tool.integrationEase}</span>
                <span class="result-bestfor">${tool.bestFor}</span>
                <span class="expand-icon">&#9660;</span>
            </div>
            <div class="result-expanded">
                <div class="expanded-grid">
                    <div class="expanded-col">
                        <h4>Pros</h4>
                        <ul class="pros-list">${tool.pros.map(p => `<li>${p}</li>`).join('')}</ul>
                    </div>
                    <div class="expanded-col">
                        <h4>Cons</h4>
                        <ul class="cons-list">${tool.cons.map(c => `<li>${c}</li>`).join('')}</ul>
                    </div>
                    <div class="expanded-col">
                        <h4>Integrations</h4>
                        <ul class="integrations-list">${tool.integrations.map(ig => `<li>${ig}</li>`).join('')}</ul>
                    </div>
                </div>
            </div>
        </div>`;
    }).join('');

    // Animate score bars
    setTimeout(() => {
        container.querySelectorAll('.score-bar-fill').forEach(bar => {
            bar.style.width = bar.getAttribute('data-target-width');
        });
    }, 200);
}

// Spotlight
function renderSpotlight() {
    const spotlightTool = scoredTools[0];
    const container = document.getElementById('spotlight-section');
    
    container.innerHTML = `
        <span class="spotlight-badge">Recommended Starting Point</span>
        <h3 class="spotlight-title">${spotlightTool.name}</h3>
        <p class="spotlight-desc">${spotlightTool.description}</p>
        <div class="spotlight-details">
            <div class="spotlight-detail">
                <span class="spotlight-detail-label">Your Fit Score</span>
                <span class="spotlight-detail-value" style="color: ${getScoreColor(spotlightTool.score)}">${spotlightTool.score}/100</span>
            </div>
            <div class="spotlight-detail">
                <span class="spotlight-detail-label">Pricing</span>
                <span class="spotlight-detail-value">${spotlightTool.pricing}</span>
            </div>
            <div class="spotlight-detail">
                <span class="spotlight-detail-label">Setup Difficulty</span>
                <span class="spotlight-detail-value">${spotlightTool.integrationEase}</span>
            </div>
            <div class="spotlight-detail">
                <span class="spotlight-detail-label">Best For</span>
                <span class="spotlight-detail-value">${spotlightTool.bestFor}</span>
            </div>
        </div>
        <p class="spotlight-note">Based on your answers and CRS365 tool data reviewed ${TOOL_DATA_REVIEWED}.</p>
    `;
}

// CTA
function renderCTA() {
    const topTool = scoredTools[0];
    const container = document.getElementById('results-cta');
    
    container.innerHTML = `
        <div class="cta-content">
            <div class="cta-match-label">Your #1 Match</div>
            <h3 class="cta-match-title">${topTool.name} with a <span class="cta-score">${topTool.score}</span> CRS Fit Score</h3>
            <p class="cta-desc">Book a free 30-minute consultation to review your results and discuss practical next steps for your business workflow.</p>
            <div class="cta-buttons">
                <a href="https://calendly.com/chadshoop/30-minute-consult" target="_blank" rel="noopener noreferrer" class="btn-cta-primary">
                    Book Free Consultation
                    <svg width="18" height="18" viewBox="0 0 20 20" fill="none"><path d="M4 10h12m0 0l-4-4m4 4l-4 4" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
                </a>
                <button class="btn-cta-secondary" onclick="downloadPDF()">
                    Download Your Results as PDF
                    <svg width="18" height="18" viewBox="0 0 20 20" fill="none"><path d="M10 3v10m0 0l-3-3m3 3l3-3M4 17h12" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
                </button>
            </div>
        </div>
    `;
}

// ============ SCORE ANIMATIONS ============
function animateScores() {
    // Animate gauge fills
    document.querySelectorAll('.score-gauge-fill').forEach(circle => {
        const targetOffset = circle.getAttribute('data-target-offset');
        setTimeout(() => {
            circle.style.strokeDashoffset = targetOffset;
        }, 100);
    });

    // Animate score numbers
    document.querySelectorAll('.score-gauge-text').forEach(el => {
        const target = parseInt(el.getAttribute('data-target'));
        animateNumber(el, 0, target, 1200);
    });
}

function animateNumber(el, from, to, duration) {
    const start = performance.now();
    function update(now) {
        const elapsed = now - start;
        const progress = Math.min(elapsed / duration, 1);
        // Ease out cubic
        const eased = 1 - Math.pow(1 - progress, 3);
        const current = Math.round(from + (to - from) * eased);
        el.textContent = current;
        if (progress < 1) requestAnimationFrame(update);
    }
    requestAnimationFrame(update);
}

// ============ EXPAND ROWS ============
function toggleExpand(row) {
    row.classList.toggle('expanded');
}

// ============ COMPARE FEATURE ============
function toggleCompare(toolId, checked) {
    if (checked) {
        if (selectedForCompare.length >= 3) {
            // Uncheck - max 3
            const cb = document.querySelector(`.result-row[data-tool-id="${toolId}"] .result-checkbox`);
            if (cb) cb.checked = false;
            return;
        }
        selectedForCompare.push(toolId);
    } else {
        selectedForCompare = selectedForCompare.filter(id => id !== toolId);
    }
    updateCompareButton();
}

function updateCompareButton() {
    const btn = document.getElementById('compare-btn');
    btn.textContent = `Compare (${selectedForCompare.length})`;
    btn.disabled = selectedForCompare.length < 2;
}

document.getElementById('compare-btn').addEventListener('click', () => {
    if (selectedForCompare.length < 2) return;
    openComparisonModal();
});

function openComparisonModal() {
    const modal = document.getElementById('comparison-modal');
    const body = document.getElementById('comparison-body');
    
    const tools = selectedForCompare.map(id => scoredTools.find(t => t.id === id)).filter(Boolean);
    
    const rows = [
        { label: 'CRS Fit Score', key: 'score', format: (t) => `<span class="comparison-score" style="color:${getScoreColor(t.score)}">${t.score}</span>` },
        { label: 'Category', key: 'category', format: (t) => t.category },
        { label: 'Pricing', key: 'pricing', format: (t) => t.pricing },
        { label: 'Integration Ease', key: 'integrationEase', format: (t) => `<span class="result-ease ${getEaseClass(t.integrationEase)}" style="display:inline-block">${t.integrationEase}</span>` },
        { label: 'Free Tier', key: 'hasFreeTier', format: (t) => t.hasFreeTier ? 'Yes' : 'No' },
        { label: 'Pros', key: 'pros', format: (t) => `<ul class="pros-list" style="margin:0">${t.pros.map(p => `<li>${p}</li>`).join('')}</ul>` },
        { label: 'Cons', key: 'cons', format: (t) => `<ul class="cons-list" style="margin:0">${t.cons.map(c => `<li>${c}</li>`).join('')}</ul>` },
        { label: 'Integrations', key: 'integrations', format: (t) => t.integrations.join(', ') }
    ];
    
    body.innerHTML = `
        <table class="comparison-table">
            <thead>
                <tr>
                    <th></th>
                    ${tools.map(t => `<th class="tool-header">${t.name}</th>`).join('')}
                </tr>
            </thead>
            <tbody>
                ${rows.map(row => `
                    <tr>
                        <td style="font-weight:600;color:var(--text-muted);font-size:13px;text-transform:uppercase;letter-spacing:0.5px;">${row.label}</td>
                        ${tools.map(t => `<td>${row.format(t)}</td>`).join('')}
                    </tr>
                `).join('')}
            </tbody>
        </table>
    `;
    
    modal.classList.add('active');
}

document.getElementById('close-compare').addEventListener('click', () => {
    document.getElementById('comparison-modal').classList.remove('active');
});

document.querySelector('.comparison-overlay').addEventListener('click', () => {
    document.getElementById('comparison-modal').classList.remove('active');
});

// ============ FILTER & SORT ============
document.querySelectorAll('.filter-tab').forEach(tab => {
    tab.addEventListener('click', () => {
        document.querySelectorAll('.filter-tab').forEach(t => t.classList.remove('active'));
        tab.classList.add('active');
        activeFilter = tab.getAttribute('data-filter');
        renderResultsGrid();
    });
});

document.getElementById('sort-select').addEventListener('change', function() {
    activeSort = this.value;
    renderResultsGrid();
});

const CRS_LOGO_B64 = 'data:image/png;base64,';

// ============ PDF GENERATION ============
function downloadPDF() {
    const { jsPDF } = window.jspdf;
    const doc = new jsPDF();
    const pageWidth = doc.internal.pageSize.getWidth();
    
    // Colors
    const navy = [26, 58, 92];
    const teal = [13, 79, 79];
    const white = [255, 255, 255];
    const lightBg = [240, 244, 255];
    
    // Header background
    doc.setFillColor(...navy);
    doc.rect(0, 0, pageWidth, 40, 'F');
    
    // Logo image
    try {
        doc.addImage(CRS_LOGO_B64, 'PNG', 14, 10, 20, 20);
    } catch (e) {
        // Fallback to text if image fails
        doc.setFillColor(...teal);
        doc.circle(25, 20, 10, 'F');
        doc.setTextColor(...white);
        doc.setFontSize(8);
        doc.setFont('helvetica', 'bold');
        doc.text('CRS', 25, 21.5, { align: 'center' });
    }
    
    // Title
    doc.setFontSize(20);
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(...white);
    doc.text('CRS365', 40, 17);
    
    doc.setFontSize(11);
    doc.setFont('helvetica', 'normal');
    doc.text('Your Personalized Tool Recommendations', 40, 25);
    
    // Date
    doc.setFontSize(9);
    doc.text(`Generated: ${new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}`, 40, 32);
    
    let y = 52;
    
    // Profile Summary
    doc.setFillColor(...lightBg);
    doc.roundedRect(14, y - 4, pageWidth - 28, 42, 3, 3, 'F');
    
    doc.setTextColor(...navy);
    doc.setFontSize(13);
    doc.setFont('helvetica', 'bold');
    doc.text('Your Business Profile', 20, y + 4);
    
    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(74, 101, 128);
    
    const profileLines = [
        `Company Size: ${formData.companySize}`,
        `Industry: ${formData.industry}`,
        `Budget: ${formData.budget}`,
        `Technical Level: ${formData.techLevel}`,
        `Challenges: ${formData.challenges.join(', ')}`
    ];
    
    y += 12;
    profileLines.forEach(line => {
        const maxWidth = pageWidth - 48;
        const wrappedLines = doc.splitTextToSize(line, maxWidth);
        wrappedLines.forEach(wl => {
            doc.text(wl, 20, y);
            y += 5.5;
        });
    });
    
    y += 10;
    
    // Top Matches Table
    doc.setTextColor(...navy);
    doc.setFontSize(13);
    doc.setFont('helvetica', 'bold');
    doc.text('Top Recommendations', 14, y);
    y += 8;
    
    const tableData = scoredTools.slice(0, 10).map((tool, i) => [
        `#${i + 1}`,
        tool.name,
        tool.category,
        `${tool.score}/100`,
        tool.pricing,
        tool.integrationEase
    ]);
    
    doc.autoTable({
        startY: y,
        head: [['Rank', 'Tool', 'Category', 'Fit Score', 'Pricing', 'Ease']],
        body: tableData,
        theme: 'grid',
        headStyles: {
            fillColor: navy,
            textColor: white,
            fontStyle: 'bold',
            fontSize: 9
        },
        bodyStyles: {
            fontSize: 9,
            textColor: [50, 50, 50]
        },
        alternateRowStyles: {
            fillColor: [245, 248, 255]
        },
        margin: { left: 14, right: 14 },
        columnStyles: {
            0: { cellWidth: 15, halign: 'center' },
            3: { halign: 'center', fontStyle: 'bold' },
            5: { halign: 'center' }
        }
    });
    
    y = doc.lastAutoTable.finalY + 16;
    
    // Check if we need a new page for CTA
    if (y > 240) {
        doc.addPage();
        y = 20;
    }
    
    // CTA Box
    doc.setFillColor(...teal);
    doc.roundedRect(14, y, pageWidth - 28, 30, 3, 3, 'F');
    
    doc.setTextColor(...white);
    doc.setFontSize(12);
    doc.setFont('helvetica', 'bold');
    doc.text('Ready to implement your stack?', pageWidth / 2, y + 12, { align: 'center' });
    
    doc.setFontSize(9);
    doc.setFont('helvetica', 'normal');
    doc.text('Book your free 30-minute consultation:', pageWidth / 2, y + 19, { align: 'center' });
    doc.setTextColor(180, 230, 230);
    doc.text('https://calendly.com/chadshoop/30-minute-consult', pageWidth / 2, y + 25, { align: 'center' });
    
    // Footer
    const footerY = doc.internal.pageSize.getHeight() - 10;
    doc.setTextColor(150, 150, 150);
    doc.setFontSize(8);
    doc.text('Generated by CRS365.com - Optimize. Automate. Operate.', pageWidth / 2, footerY, { align: 'center' });
    
    // Generate blob and download
    const pdfBlob = doc.output('blob');
    const url = URL.createObjectURL(pdfBlob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'CRS365-Tool-Recommendations.pdf';
    a.click();
    URL.revokeObjectURL(url);
}

// ============ MAKE FUNCTIONS GLOBAL ============
window.toggleExpand = toggleExpand;
window.toggleCompare = toggleCompare;
window.downloadPDF = downloadPDF;

// ============ ADMIN PANEL ============
const ADMIN_PW = 'crs365admin';
let adminData = [];

function showAdminLogin() {
    document.getElementById('admin-overlay').style.display = 'flex';
    document.getElementById('admin-login').style.display = 'block';
    document.getElementById('admin-content').style.display = 'none';
    document.getElementById('admin-error').style.display = 'none';
    document.getElementById('admin-pw').value = '';
    document.getElementById('admin-pw').focus();
}

function closeAdmin() {
    document.getElementById('admin-overlay').style.display = 'none';
}

function adminAuth() {
    const pw = document.getElementById('admin-pw').value;
    if (pw === ADMIN_PW) {
        document.getElementById('admin-login').style.display = 'none';
        document.getElementById('admin-content').style.display = 'flex';
        refreshAdmin();
    } else {
        document.getElementById('admin-error').style.display = 'block';
    }
}

function refreshAdmin() {
    // Reads submissions from localStorage.
    // NOTE: localStorage is per-browser/device. Submissions made in other
    // browsers or devices will not appear here. For a central database,
    // connect the Brevo proxy (see workers/brevo-proxy.js) and use the
    // Brevo dashboard as your canonical subscriber list.
    try {
        adminData = JSON.parse(localStorage.getItem('crs_submissions') || '[]');
        renderAdminTable(adminData);
    } catch (err) {
        console.error('Failed to read submissions from localStorage:', err);
        adminData = [];
        renderAdminTable(adminData);
    }
}

function renderAdminTable(data) {
    const tbody = document.getElementById('admin-tbody');
    document.getElementById('admin-count').textContent = `${data.length} submission${data.length !== 1 ? 's' : ''}`;

    if (data.length === 0) {
        tbody.innerHTML = '<tr><td colspan="12" style="text-align:center;padding:32px;color:var(--text-muted);">No submissions yet.</td></tr>';
        return;
    }

    tbody.innerHTML = data.map((s, i) => {
        const challenges = Array.isArray(s.challenges) ? s.challenges.join(', ') : (s.challenges || '');
        const tools = Array.isArray(s.current_tools) ? s.current_tools.join(', ') : (s.current_tools || '');
        const date = s.submitted_at ? new Date(s.submitted_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric', hour: 'numeric', minute: '2-digit' }) : 'N/A';
        const hasConsent = s.email_consent ?? s.newsletter_opt_in;
        const nlBadge = hasConsent ? '<span class="admin-badge-yes">Yes</span>' : '<span class="admin-badge-no">No</span>';

        return `<tr>
            <td>${s.id || i + 1}</td>
            <td>${date}</td>
            <td><strong>${s.name || ''}</strong></td>
            <td>${s.email || ''}</td>
            <td>${s.company || ''}</td>
            <td>${s.company_size || ''}</td>
            <td>${s.industry || ''}</td>
            <td class="challenges-cell">${challenges}</td>
            <td>${s.budget || ''}</td>
            <td class="tools-cell">${tools}</td>
            <td>${s.tech_level || ''}</td>
            <td>${nlBadge}</td>
        </tr>`;
    }).join('');
}

function exportCSV() {
    if (adminData.length === 0) return;

    const headers = ['ID','Date','Name','Email','Company','Company Size','Industry','Challenges','Budget','Current Tools','Tech Level','Consent'];
    const rows = adminData.map(s => [
        s.id,
        s.submitted_at || '',
        s.name || '',
        s.email || '',
        s.company || '',
        s.company_size || '',
        s.industry || '',
        Array.isArray(s.challenges) ? s.challenges.join('; ') : (s.challenges || ''),
        s.budget || '',
        Array.isArray(s.current_tools) ? s.current_tools.join('; ') : (s.current_tools || ''),
        s.tech_level || '',
        (s.email_consent ?? s.newsletter_opt_in) ? 'Yes' : 'No'
    ]);

    const csv = [headers, ...rows].map(row =>
        row.map(cell => `"${String(cell).replace(/"/g, '""')}"`).join(',')
    ).join('\n');

    const blob = new Blob([csv], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `CRS365-Leads-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
    URL.revokeObjectURL(url);
}

// Close admin on overlay click
document.getElementById('admin-overlay').addEventListener('click', function(e) {
    if (e.target === this) closeAdmin();
});

window.showAdminLogin = showAdminLogin;
window.closeAdmin = closeAdmin;
window.adminAuth = adminAuth;
window.refreshAdmin = refreshAdmin;
window.exportCSV = exportCSV;









